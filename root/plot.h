...
void drawText(const char *text, float xp, float yp, int size, int color = 1){
...
void putSTARPrel(double x=0.2, double y=0.95, double size=0.05){
  TLatex *tex=0;
  tex = new TLatex(x,y,"STAR Preliminary");
  tex->SetTextSize(size);
  tex->SetLineWidth(2);
  tex->SetNDC();
  tex->Draw();
...
void putAuAuLumi(double x=0.2, double y=0.95, double size=0.05){
  TLatex *tex=0;
  tex = new TLatex(x,y,"AuAU #int L dt = pb^{-1}");
  tex->SetTextSize(size);
  tex->SetLineWidth(2);
  tex->SetNDC();
  tex->Draw();
...
void putPPLumi(double x=0.2, double y=0.95, double size=0.05){
  TLatex *tex=0;
  tex = new TLatex(x,y,"pp #int L dt =  pb^{-1}");
  tex->SetTextSize(size);
  tex->SetLineWidth(2);
  tex->SetNDC();
  tex->Draw();
...
void putSTARSim(double x=0.17, double y=0.93, double size=0.08){
  TLatex *tex=0;
  tex = new TLatex(x,y,"STAR Simulation");
  tex->SetTextSize(size);
  tex->SetLineWidth(2);
  tex->SetNDC();
  tex->Draw();
...
TH1F* drawEnvelope2(TH1F *h,char *opt,int color = kGray,int fillStyle = 0, int fillColor = 0,double shift = 0)
{
  TH1F *hClone = (TH1F*) h->Clone(Form("%s_mirror",h->GetTitle()));
  TH1F *hMirror = (TH1F*) h->Clone(Form("%s_mirror2",h->GetTitle()));
  for (int i=1;i<=h->GetNbinsX();i++)
    {
      double val = h->GetBinContent(i);
      hMirror->SetBinContent(i,1-fabs(val-1)+shift);
      hClone->SetBinContent(i,val);
    }
  
  //   hMirror->SetLineStyle(2);
  //   h->SetLineStyle(2);
  hMirror->SetLineColor(0);
  hMirror->SetFillColor(0);
  hMirror->SetFillStyle(fillStyle);
  hClone->SetLineColor(color);
  hClone->SetFillColor(fillColor);
  hClone->SetFillStyle(fillStyle);
...
void putInfoOnCanvas(){

  drawText("#gamma+Jet Events, anti-k_{t} R=0.4 Jets",0.5, 0.85, 18);
  drawText("p^{#gamma}_{T} > 100 GeV/c, p^{Jet}_{T} > 50 GeV/c",0.5, 0.78, 18);
  drawText("#Delta #phi > 2#pi/3, |#eta^{#gamma,Jet}| < 1.5",0.5, 0.71, 18);
  
}

void putppInfo(){
  drawText("Grid-Jet (#Delta#eta,#Delta#phi = 0.08)", 0.5, 0.85, 18);
}

void putpbpbInfo(){
  drawText("w/ Recoils Grid-Sub (#Delta#eta,#Delta#phi = 0.08)", 0.45, 0.78, 18);  
}
