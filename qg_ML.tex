...
We combine the five jet substructure observables using a multivariate model implemented in Keras \cite{keras}. The details of the model description are given in Appendix~\ref{app:models}. 
...
A deep convolutional neural network is implemented in Keras~\cite{DBLP:journals/corr/AbadiBCCDDDGIIK16}. The neural network architecture and the corresponding activation functions are detailed in the appendix. 
...
\begin{figure}[h]
	...
	\caption{ROC curves of quark v.s. gluon efficiency with the DCNN method highlighting the effect of the embedded \textsc{Hydjet} background shown in the red curves in comparison to the blue curves which are in vacuum at the generator level. The solid and dotted  curves correspond to PbPb quenched and pp events respectively.}
	\label{fig:ROC_qvsg_wHydjet}
\end{figure}

The fluctuating underlying event and their subtraction in experiment can both influence and introduce features in jet substructure. We employ the embedding technique which is used in almost all experiments to estimate the effect of these fluctuations on an event by event basis. We take an unbiased heavy ion event produced by the \textsc{Hydjet}~\cite{Lokhtin:2008xi} MC for most central collisions ($0-5\%$) and embed the particles produced by \jw (both vacuum and in-medium) together. In this embedded event, we have included the effect of the uncorrelated background to jets. Since the \jw events with jet quenching are for a centrality window of $0-20\%$, this underlying event presents an over estimate of the background which we take as an upper bound for the effects due to the underlying event. In order to extract jets from these embedded events, we perform constituent subtraction~\cite{Berta:2014eza} which reduces the contribution of the background to the jet and its constituents. The effect of including the heavy ion underlying event is shown in Figure~\ref{fig:ROC_qvsg_wHydjet} in the dashed curves having an overall reduction in DCNN classification performance as shown by an increased auc for both vacuum jets and quenched jets but we would like to note that classification is still experimentally viable albeit with reduced efficiency. We test the DCNN method for this embedding study since it is agnostic to jet substructure artifacts introduced during the embedding and background subtraction procedures. Further studies of the exact nature of the background influence on jet substructure via detector simulations are reserved for future publications.   
...
\section{Description of the MLP and CNN network architectures}
\label{app:models}
The MLP utilized in this classification analysis utilizes two hidden dense layers of 10 nodes each as shown in Fig~\ref{fig:mlp}. We use the rectified linear unit (ReLU) \cite{nair2010rectified} and sigmoid activation functions for the dense layers and the output layer, respectively. The model training utilizes the Adam optimizer \cite{adam} and the binary cross entropy loss function. We perform cross validation using Monte Carlo samples which are split into two random halves for training and testing.

\begin{figure}[t]
\centering
\includegraphics[width=0.6\columnwidth]{FigMLP.png}
\caption{Schematic of the MLP utilized for physics observables where the two hidden layers are shown and particular nodes that are not linked are dropped (shown as an example) during training.}
\label{fig:mlp}
\end{figure}

The CNN architecture consists of three 48-filter convolutional layers with filter sizes of $8\times 8$, $4\times 4$, and $4\times 4$ followed by a 128-unit dense layer and a 2-unit softmaxed output layer. The network architecture is schematically shown in Fig~\ref{fig:dcnn}. A $2\times 2$ maxpooling is performed after each convolutional layer with a stride length of 2. The dropout rate was taken to be 0.1 for all layers. All neural networks are implemented using the Python deep learning library Keras~\cite{keras} with the Theano backend~\cite{bergstra2010theano}. Rectified linear unit (ReLU) activation functions~\cite{nair2010rectified} and He-uniform model weight initialization~\cite{heuniform} are used. The networks are trained using the Adam algorithm~\cite{adam} with a learning rate of $10^{-3}$ and a batch size of 256 for 50 epochs with a patience parameter of 8, and the best model is selected based on validation set performance.

\begin{figure}[t]
\centering
\includegraphics[width=.8\columnwidth]{FigDCNN_wolabels.png}
\caption{Schematic of the DCNN with maxpooling layers (not shown) located in-between each convolution layer. The output layer for classification is taken as a result of a dense layer (not shown).}
\label{fig:dcnn}
\end{figure}
...
A neural network consisting of three dense layers with 100 nodes each is trained on the telescoping deconstruction observables up to T$N$ order. The training can be performed on a typical laptop CPU in fewer than five minutes. One could also use boosted decision trees (BDT) to combine variables \cite{Chien:2017xrb}. We compare the performance of our method to jet images using a CNN as done in the main text. The jet images are size $33\times 33$ and span a $2R\times 2R$ patch of the rapidity-azimuth plane with the intensity of each pixel corresponding to the total $p_T$ of particles in the pixel. The jet images are pre-processed and standardized according to the procedure in \Ref{Komiske:2016rsd}. 
